...
namespace QV.Web.Controllers
{
    public class PortletController : Controller
    {
        ...
        public ActionResult PortletsPlacementManager(string column_ID, string portlet_ID, int? row_No, int? is_Drop)
        {
            ...
            #region declaration
            int _portlet_ID;
                int.TryParse(portlet_ID.ToString().Replace("portlet", ""),out _portlet_ID);
            int category_ID; 
                 int.TryParse(column_ID.Replace("portletColumn", "").Substring(1),out category_ID);

            int column_No;
            int.TryParse(column_ID.Replace("portletColumn", "").Substring(0, 1),out column_No);
            ...
            if (rows.Count == 1)
            {
                ...
                if (row_No.HasValue) firstRow.Row_Sequence=row_No.Value;
                ...
                if (is_Drop == 0)
                {
                    int subsequent_Row_No= (row_No.HasValue)?row_No.Value+1:0;
        ...
        {
            int _category_ID;
            int.TryParse(category_ID.Replace("portletColumn", "").Substring(1),out _category_ID);
